...
public:
  ...
  int64_t get_rank(const std::string &member)
      const; // ZRANK (0-indexed position in sorted order)

  // Sorted Range Scans (Traverses Internal Compound AVL Tree)
  size_t size() const; // ZCARD

  // Returns elements ordered by index rank (e.g., top 10 players)
  std::vector<std::pair<std::string, double>>
  get_range_by_rank(int64_t start, int64_t end) const; // ZRANGE

  // Returns elements bounded by numeric score criteria (e.g., scores 100 to
  // 500)
  std::vector<std::pair<std::string, double>>
  get_range_by_score(double min_score, double max_score) const; // ZRANGEBYSCORE

  void delete_complete_obj() { delete this; }
};
